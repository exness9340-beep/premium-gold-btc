...
def generate_signal_chart(df, signal, sweep, bias, asset_key):
    ...
    # Labels & styling
    direction_symbol = "▲ BUY" if bias == "BUY" else "▼ SELL"
    ax.set_title(
...
def format_signal(asset_key, signal, sweep, session, bias):
    ...
    # Fib direction line — clearly show which swing to which swing
...
class FibAgent:
    def __init__(self):
        ...
        # Confirmation counters — SL/TP must be confirmed 3 times before triggering
        ...
        if sweep:
            ...
            if signal:
                ...
                try:
                    ...
                    print(f"[CHART ERROR] {chart_err} — sending text only")
                    send_telegram(msg)
